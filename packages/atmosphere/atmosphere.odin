...
lerp_weather :: proc(a, b: Weather_State, amount: f32) -> Weather_State {
	...
	return {
		...
		wind = {
			a.wind[0] + (b.wind[0] - a.wind[0]) * t,
			a.wind[1] + (b.wind[1] - a.wind[1]) * t,
		},
	}
}
...
step :: proc(state: ^Atmosphere, delta_seconds: f32) {
	...
	if !state.paused {
		state.world_minutes = f32(math.mod(
			f64(state.world_minutes + delta * WORLD_MINUTES_PER_SECOND),
			f64(DAY_MINUTES),
		))
...
sample :: proc(state: ^Atmosphere) -> Sky_State {
	...
	sun := [3]f32{
		f32(math.cos(f64(angle))) * .72,
		f32(math.sin(f64(angle))),
		f32(math.cos(f64(angle))) * .38,
	}
