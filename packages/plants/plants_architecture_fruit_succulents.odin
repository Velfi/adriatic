...
lemon_architecture :: proc(seed: u64, maturity: f32, generations: int) -> plant_structure.Interpret_Result {
    ...
    for index in 0 ..< trunk_segments {
        ...
        drift :=
            plant_structure.Vec3{olive_random_signed(&random) * .025, .34, olive_random_signed(&random) * .025} * scale
    ...
    for scaffold_index in 0 ..< scaffold_count {
        ...
        direction := linalg.normalize0(
            radial * (.72 + olive_random_signed(&random) * .08) + plant_structure.Vec3{0, rise, 0},
        )
...
succulent_catalog_architecture :: proc(
    ...
    if species == .Jade_Plant {
        // Preserve an open, legible crown: jade is a branching succulent,
        // not a topiary mass. Lower tiers retain opposing leaf pairs while
        // later LODs remove whole stable stems and terminal nodes.
    ...
    if species == .Blue_Chalk_Sticks {
        ...
        for clump_index in 0 ..< clump_count {
            ...
            center := plant_structure.Vec3 {
                math.cos(runner_angle) * runner_distance,
                .008,
                math.sin(runner_angle) * runner_distance,
            }
            if clump_index == 0 {
...
strawberry_tree_architecture :: proc(seed: u64, maturity: f32, generations: int) -> plant_structure.Interpret_Result {
    ...
    if maturity > .30 {
        ...
        for leaf, leaf_index in result.plant.leaves[:original_leaf_count] {
            ...
            append(
                &result.plant.leaves,
                plant_structure.Leaf{position = leaf.position, forward = {0, -1, 0}, up = leaf.up, depth = -12},
            )
        }
    }
    return result
