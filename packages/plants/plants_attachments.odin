...
fold_source_attachment_frame :: proc(
    forward, up: ^plant_structure.Vec3,
...
update_leaf_bounds :: proc(
    bounds: ^Bounds,
    position, forward, up: plant_structure.Vec3,
    traits: Leaf_Traits,
    first: ^bool,
) {
...
finalize_segment_topology :: proc(plant: ^Generated_Plant, seed: u64, species: Species) {
    ...
    plant.segment_ids = make([dynamic]u64, count)
    resize(&plant.segment_parents, count)
    resize(&plant.segment_axes, count)
    resize(&plant.segment_ids, count)
    axis_count := 0
    for segment, segment_index in plant.segments {
        ...
        plant.segment_ids[segment_index] = generated_stable_id(seed, 0x5345474d454e54, segment_index)
    ...
    plant.axis_roles = make([dynamic]Axis_Role, axis_count)
    plant.axis_orientations = make([dynamic]Axis_Orientation, axis_count)
    resize(&plant.axis_parents, axis_count)
    resize(&plant.axis_roles, axis_count)
    resize(&plant.axis_orientations, axis_count)
    for &parent in plant.axis_parents do parent = -1
    profile := garden_profile(species)
    for segment, segment_index in plant.segments {
        axis := plant.segment_axes[segment_index]
        parent_segment := plant.segment_parents[segment_index]
        if parent_segment >= 0 {
            parent_axis := plant.segment_axes[parent_segment]
            if parent_axis != axis do plant.axis_parents[axis] = parent_axis
        }
        direction := linalg.normalize0(segment.end - segment.start)
        plant.axis_orientations[axis] = math.abs(direction[1]) > .72 ? .Orthotropic : .Plagiotropic
    }
    for axis in 0 ..< axis_count {
        ...
        case .Tendril_Climber, .Twining_Climber, .Scrambling_Climber:
            role = .Climber
        case .Cushion:
            role = .Runner
        case .Rosette,
        ...
        plant.axis_roles[axis] = role
    }
    plant.attachment_ids = make([dynamic]u64, len(plant.attachments))
    resize(&plant.attachment_ids, len(plant.attachments))
    for index in 0 ..< len(plant.attachments) {
        plant.attachment_ids[index] = generated_stable_id(seed, 0x415454414348, index)
    }
    generated_graph_build(plant, seed)
}
