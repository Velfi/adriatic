...
pelargonium_architecture :: proc(seed: u64, maturity: f32) -> plant_structure.Interpret_Result {
    ...
    for stem_index in 0 ..< stem_count {
        ...
        stem_lean := .34 + f32(stem_index % 4) * .035
        direction := linalg.normalize0(
            radial * stem_lean + tangent * olive_random_signed(&random) * .08 + plant_structure.Vec3{0, .90, 0},
        )
        ...
        for node_index in 0 ..< node_count {
            ...
            length :=
                (.055 + eased * .030) * (1 - node_progress * .08) * (1 + olive_random_signed(&random) * .08) * size
            direction = linalg.normalize0(
        ...
        if growth >= .30 && stem_index % 2 == 0 {
            ...
            append(
                &result.plant.leaves,
                plant_structure.Leaf{position = flower_tip, forward = {0, 1, 0}, up = radial, depth = -4},
            )
        }
    }
    return result
    ...
    for stem_index in 0 ..< stem_count {
        ...
        for segment_index in 0 ..< segment_count {
            ...
            leaf_normal := linalg.normalize0(
                plant_structure.Vec3{0, .72, 0} + radial * (.62 + olive_random_signed(&random) * .10),
            )
            append(
                &result.plant.leaves,
        ...
        if maturity > .22 {
            ...
            append(
                &result.plant.leaves,
                plant_structure.Leaf {
                    position = position + plant_structure.Vec3{0, .042 * growth, 0},
                    // Terminal heads remain predominantly upright, with a
                    // small radial lean that breaks the nursery-perfect row.
                    forward  = linalg.normalize0(radial * .12 + plant_structure.Vec3{0, .99, 0}),
                    up       = radial,
                    depth    = -9,
                },
            )
...
rosemary_architecture :: proc(seed: u64, maturity: f32, detail: Detail_Level) -> plant_structure.Interpret_Result {
    ...
    stem_count := detail == .Near ? 16 : detail == .Medium ? 12 : 8
    phase := f32(plant_structure.random_next(&random) % 10_000) / 10_000 * math.PI * 2
    leaf_fractions := [3]f32{.18, .52, .84}
    for stem_index in 0 ..< stem_count {
        ...
        segment_count := 4 + stem_index % 3
        for segment_index in 0 ..< segment_count {
            direction := linalg.normalize0(
                radial * (.18 + f32(segment_index) * .026) +
                tangent * olive_random_signed(&random) * .10 +
            ...
            for fraction, fraction_index in leaf_fractions {
                ...
                outward := plant_structure.Vec3{math.cos(leaf_azimuth), .10, math.sin(leaf_azimuth)}
                append(
                    &result.plant.leaves,
                    plant_structure.Leaf {
                        position = leaf_position,
            ...
            if segment_index >= 1 && segment_index <= 3 && (stem_index + segment_index) % 2 == 0 {
                ...
                for fraction in leaf_fractions {
                    append(
                        &result.plant.leaves,
                        plant_structure.Leaf {
                            position = linalg.lerp(next, side_end, fraction),
                            forward = linalg.normalize0(tangent * side_sign + plant_structure.Vec3{0, .12, 0}),
                            up = radial,
                            depth = segment_index + 1,
                        },
                    )
...
grapevine_architecture :: proc(seed: u64, maturity: f32, detail: Detail_Level) -> plant_structure.Interpret_Result {
    ...
    trunk_mid_a := plant_structure.Vec3{.010 * olive_random_signed(&random), cordon_y * .34, .008}
    trunk_mid_b := plant_structure.Vec3{-.008 * olive_random_signed(&random), cordon_y * .70, -.006}
    append(
        &result.plant.segments,
        plant_structure.Segment{{0, 0, 0}, trunk_mid_a, .050, .040, -23},
        plant_structure.Segment{trunk_mid_a, trunk_mid_b, .040, .031, -23},
        plant_structure.Segment{trunk_mid_b, {0, cordon_y, 0}, .031, .024, -23},
    )
    cordon_links := 12
    for link_index in 0 ..< cordon_links {
        ...
        append(
            &result.plant.segments,
            plant_structure.Segment{{x0, y0, 0}, {x1, y1, 0}, radius, max(radius * .94, f32(.010)), -20},
        )
    }
    ...
    for spur_index in 0 ..< spur_count {
        ...
        cordon_point := plant_structure.Vec3{x, local_cordon_y, 0}
        spur_side := spur_index % 2 == 0 ? f32(-1) : f32(1)
        spur_tip := cordon_point + plant_structure.Vec3{spur_side * .012, .040 * growth, .010 * spur_side}
        append(&result.plant.segments, plant_structure.Segment{cordon_point, spur_tip, .011, .008, -22})

        active_threshold := .38 + clamp(maturity, f32(0), f32(1)) * .57
        if f32(plant_structure.random_next(&random) % 10_000) / 10_000 > active_threshold do continue
        annual_count := detail == .Near && plant_structure.random_next(&random) % 5 == 0 ? 2 : 1
        for annual_index in 0 ..< annual_count {
            phytomer_count :=
                detail == .Near ? 6 + int(plant_structure.random_next(&random) % 4) : detail == .Medium ? 5 + int(plant_structure.random_next(&random) % 3) : 4 + int(plant_structure.random_next(&random) % 2)
            position := spur_tip
            ...
            for phytomer_index in 0 ..< phytomer_count {
                ...
                direction := linalg.normalize0(plant_structure.Vec3{lateral_velocity, 1, depth_velocity})
                next := position + direction * internode_length
                append(&result.plant.segments, plant_structure.Segment{position, next, radius, radius * .86, -21})
                leaf_side := phytomer_index % 2 == 0 ? f32(-1) : f32(1)
                append(
                    &result.plant.leaves,
                    plant_structure.Leaf {
                        position = next,
                        forward = linalg.normalize0(plant_structure.Vec3{leaf_side * .78, .28, .18}),
                        up = {0, 0, 1},
                        depth = -21,
                    },
                )
...
star_jasmine_architecture :: proc(seed: u64, maturity: f32, detail: Detail_Level) -> plant_structure.Interpret_Result {
    ...
    cane_points: [9][6]plant_structure.Vec3
    for cane_index in 0 ..< cane_count {
        ...
        lateral_reach := .72 + f32(plant_structure.random_next(&random) % 10_000) / 10_000 * .48
        ...
        for level_index in 0 ..< level_count {
            ...
            append(&result.plant.segments, plant_structure.Segment{position, next, radius, radius * .78, level_index})
            direction := linalg.normalize0(next - position)
            for fraction in leaf_fractions {
                append(
                    &result.plant.leaves,
    ...
    if detail != .Far {
        ...
        for level_index in 2 ..= 4 {
            ...
            for cane_index := pair_offset; cane_index + 1 < cane_count; cane_index += 2 {
                ...
                append(&result.plant.segments, plant_structure.Segment{start, end, .0035, .0024, level_index + 1})
...
bougainvillea_architecture :: proc(seed: u64, maturity: f32, detail: Detail_Level) -> plant_structure.Interpret_Result {
    ...
    result := star_jasmine_architecture(seed ~ 0x626f756761696e76, maturity, detail)
    cane_count := detail == .Near ? 9 : detail == .Medium ? 7 : 5
    level_count := 5
    ...
    // Author those shoots in the architecture so every renderer and LOD inherits
    ...
    for segment_index in 0 ..< cane_segment_count {
        ...
        for lateral_index in 0 ..< lateral_count {
            ...
            lateral_direction := linalg.normalize0(
                plant_structure.Vec3{side, .28 + olive_random_signed(&lateral_random) * .10, depth_offset} +
                source_direction * .18,
            )
            tip := node + lateral_direction * reach + plant_structure.Vec3{0, lift, 0}
            tip[1] = min(tip[1], structural_height)
            lateral_depth := 8 + level_index
            lateral_radius := max(source.radius_end * .42, f32(.0018))
            append(
                &result.plant.segments,
                plant_structure.Segment{node, tip, lateral_radius, lateral_radius * .46, lateral_depth},
            )
...
wisteria_architecture :: proc(seed: u64, maturity: f32, detail: Detail_Level) -> plant_structure.Interpret_Result {
    result := star_jasmine_architecture(seed ~ 0x9e3779b97f4a7c15, maturity, detail)
    ...
    if detail != .Far && maturity > .36 {
        cane_count := detail == .Near ? 9 : detail == .Medium ? 7 : 5
        level_count := 5
        for cane_index in 0 ..< cane_count {
            for level_index in 2 ..< level_count {
                if (cane_index + level_index) % 2 != 0 do continue
                segment := result.plant.segments[cane_index * level_count + level_index]
                ...
                append(
                    ...
                    plant_structure.Leaf {
                        ...
                        forward = {0, -1, 0},
                        up = linalg.normalize0(plant_structure.Vec3{-direction[2], 0, direction[0]}),
                        depth = -10,
                    },
                )
            }
...
climbing_rose_architecture :: proc(seed: u64, maturity: f32, detail: Detail_Level) -> plant_structure.Interpret_Result {
    ...
    if detail != .Far && maturity > .32 {
        cane_count := detail == .Near ? 9 : detail == .Medium ? 7 : 5
        level_count := 5
        for cane_index in 0 ..< cane_count {
            terminal := result.plant.segments[cane_index * level_count + level_count - 1]
...
sprout_newest_generation :: proc(plant: ^plant_structure.Plant, progress: f32) {
    ...
    old_segments := make([]plant_structure.Segment, len(plant.segments))
    ...
    for &segment, index in plant.segments {
        ...
        // Find an earlier segment in this shoot. Architecture generation is
...
olive_leaf_frame :: proc(direction: plant_structure.Vec3) -> (forward, up: plant_structure.Vec3) {
    forward = linalg.normalize0(direction)
    reference := math.abs(forward[1]) > .88 ? plant_structure.Vec3{1, 0, 0} : plant_structure.Vec3{0, 1, 0}
