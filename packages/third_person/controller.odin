...
step :: proc(state: ^State, input: Input, config: Config, delta_seconds: f32) {
	...
	direction := Vec3 {
		forward.x * move_y + right.x * move_x,
		0,
		forward.z * move_y + right.z * move_x,
	}
...
follow_camera :: proc(
	current: Camera_Pose,
	desired: Camera_Pose,
	sharpness, delta_seconds: f32,
) -> Camera_Pose {
	if delta_seconds <= 0 do return current
	t := clamp(sharpness * delta_seconds, 0, 1)
	return {
		position = lerp(current.position, desired.position, t),
		target = lerp(current.target, desired.target, t),
	}
}
...
scale :: proc(value: Vec3, amount: f32) -> Vec3 {return{
		value.x * amount,
		value.y * amount,
		value.z * amount,
	}}
lerp :: proc(a, b: Vec3, t: f32) -> Vec3 {return add(
		a,
		scale(Vec3{b.x - a.x, b.y - a.y, b.z - a.z}, t),
	)}
