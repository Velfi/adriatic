...
Terrain_Progress_Proc :: proc(stage: string, current, total: int)
...
init_project_seeded :: proc(
    result: ^Project,
    seeds: [len(DEFAULT_ISLAND_SEEDS)]u32,
    progress: Terrain_Progress_Proc = nil,
) {
    ...
    for level in 0 ..< CLIPMAP_LEVELS {
        if progress != nil do progress("terrain level", level + 1, CLIPMAP_LEVELS)
...
sample_authored_field_raw :: #force_inline proc(
    ...
    for candidate in level ..< CLIPMAP_LEVELS {
        ...
        material = sample_level_material(data, x, z)
