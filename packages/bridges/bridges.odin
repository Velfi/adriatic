...
defaults :: proc(archetype: Archetype = .Dalmatian_Multi_Arch) -> Config {
    ...
    case .Dalmatian_Multi_Arch:
        return {
            ...
            span_count = 8,
            material = .Travertine,
            construction = .Dry_Stone,
            arch_shape = .Segmental,
            deck_profile = .Crowned,
            parapet = .Solid,
            pier_cutwaters = true,
        }
    case .Herzegovinian_High_Arch:
        return {
            ...
            span_count = 1,
            material = .Limestone,
            construction = .Lime_Mortared,
            arch_shape = .Slightly_Pointed,
            deck_profile = .Humpback,
            parapet = .Solid,
        }
    case .Venetian_Canal:
        return {
            ...
            clearance = 5.2,
            span_count = 1,
            material = .Istrian_Stone,
            construction = .Dressed_Masonry,
            arch_shape = .Segmental,
            deck_profile = .Stepped,
            parapet = .Balustrade,
            approach_steps = true,
        }
    case .Cycladic_Rural:
        return {
            ...
            material = .Fieldstone,
            construction = .Dry_Stone,
            arch_shape = .Semicircular,
            deck_profile = .Humpback,
            parapet = .Low_Irregular,
        }
...
generate :: proc(seed: u32, requested: Config) -> Plan {
    ...
    if config.archetype == .Herzegovinian_High_Arch ||
       config.archetype == .Venetian_Canal ||
       config.archetype == .Cycladic_Rural ||
       config.archetype == .Aegean_Fortress {
        config.span_count = 1
    }
    ...
    case .Crowned:
        deck_rise = .18 + variation * .12
    case .Humpback:
        deck_rise = clamp(config.length * .075, f32(.8), f32(2.8))
    case .Stepped:
        deck_rise = clamp(config.length * .055, f32(.65), f32(1.8))
    case .Level:
    }
    ...
    plan := Plan {
        ...
        arch_rise          = min(
            config.clearance * f32(.88),
            span_length * (config.arch_shape == .Segmental ? f32(.34) : f32(.49)),
        ),
        deck_rise          = deck_rise,
