...
Color_Float :: struct {r, g, b: f32}
Vertex :: struct {position: [3]f32, color: Color_Float}
Edge :: struct {a, b: int}
Camera :: struct {position: [3]f32, right, up, forward: [3]f32, focal_length, near_plane: f32}
Target :: struct {width, height: int, pixels: []Color, depth: []f32}
Screen_Vertex :: struct {x, y, depth: f32, color: Color_Float, valid: bool}

clamp_f32 :: proc(value, low, high: f32) -> f32 { if value < low do return low; if value > high do return high; return value }
...
default_camera :: proc() -> Camera { return {right={1, 0, 0}, up={0, 1, 0}, forward={0, 0, -1}, focal_length=1, near_plane=.05} }

target_valid :: proc(target: Target) -> bool { return target.width > 0 && target.height > 0 && len(target.pixels) >= target.width*target.height && len(target.depth) >= target.width*target.height }
...
project :: proc(camera: Camera, vertex: Vertex, width, height: int) -> Screen_Vertex {
	...
	return {x=(x*.5+.5)*f32(width-1), y=(.5-y*.5)*f32(height-1), depth=depth, color=vertex.color, valid=true}
}
...
draw_line :: proc(target: ^Target, a, b: Screen_Vertex) {
	...
	for step in 0 ..< steps+1 {
		...
		target.pixels[index] = {u8(clamp_f32(lerp(a.color.r,b.color.r,t),0,1)*255), u8(clamp_f32(lerp(a.color.g,b.color.g,t),0,1)*255), u8(clamp_f32(lerp(a.color.b,b.color.b,t),0,1)*255)}
	}
}
