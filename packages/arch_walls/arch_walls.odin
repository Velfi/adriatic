...
stepped_ground :: #force_inline proc(
    sample: Terrain_Height_Proc,
    data: rawptr,
    point: Vec2,
    step_height: f32,
) -> f32 {
...
path_point :: proc(path: ^Path, amount: f32) -> Vec2 {
    ...
    t := scaled-f32(piece)
    return catmull_rom(
        path.points[path_index(path,piece-1)], path.points[path_index(path,piece)],
        path.points[path_index(path,piece+1)], path.points[path_index(path,piece+2)], t,
    )
}
...
generate :: proc(path: ^Path, config: Config, terrain_height: Terrain_Height_Proc, terrain_data: rawptr = nil) -> Plan {
    plan: Plan
    ...
    for step in 0 ..< span_count {
        ...
        append(&plan.spans, Span {
            ...
            station_from=station, station_to=station+distance,
        })
        station += distance
        previous = to
    }
        ...
        for arch_index in 0 ..< arch_count {
            ...
            for span in plan.spans {
                ...
                append(&plan.arches, Arch {
                    ...
                    station=station_target, width=config.arch_width, height=config.arch_height,
                })
                break
            }
        }
