...
clamp :: proc(value, low, high: f32) -> f32 { if value < low do return low; if value > high do return high; return value }
...
Basis :: struct {forward, up, right: Vec3}
...
local_to_world :: proc(b: Basis, local: Vec3) -> Vec3 { return add(add(scale(b.right, local.x), scale(b.up, local.y)), scale(b.forward, local.z)) }
world_to_local :: proc(b: Basis, world: Vec3) -> Vec3 { return {dot(world, b.right), dot(world, b.up), dot(world, b.forward)} }
