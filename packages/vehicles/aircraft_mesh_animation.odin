...
rotate_mesh_part_x :: proc(
	...
	for &vertex in mesh.vertices[:mesh.vertex_count] {
		...
		vertex.position[1] =
			pivot[1] + y * c - z * s; vertex.position[2] = pivot[2] + y * s + z * c
	}
}

rotate_mesh_part_y :: proc(
	mesh: ^Aircraft_Mesh,
	part: Aircraft_Mesh_Part,
	pivot: [3]f32,
	angle: f32,
	...
	for &vertex in mesh.vertices[:mesh.vertex_count] {
		...
		vertex.position[0] =
			pivot[0] + x * c + z * s; vertex.position[2] = pivot[2] - x * s + z * c
	}
}

rotate_mesh_part_z :: proc(
	mesh: ^Aircraft_Mesh,
	part: Aircraft_Mesh_Part,
	pivot: [3]f32,
	angle: f32,
	...
	for &vertex in mesh.vertices[:mesh.vertex_count] {
		...
		vertex.position[0] =
			pivot[0] + x * c - y * s; vertex.position[1] = pivot[1] + x * s + y * c
	}
}
