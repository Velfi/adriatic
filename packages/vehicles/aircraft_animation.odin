...
clamp_unit :: proc(value: f32) -> f32 {if value < 0 do return 0; if value > 1 do return 1; return(
		value \
	)}

set_vertical_propeller :: proc(
	model: ^Aircraft_Wireframe,
	center_x, center_y, center_z, radius, turns: f32,
...
animate_postale :: proc(model: ^Aircraft_Wireframe, flap_fraction, propeller_turns: f32) {
	...
	model.vertices[10].position[1] =
		.9 - .62 * flap; model.vertices[11].position[1] = .9 - .62 * flap
	model.vertices[10].position[2] =
		.65 + .28 * flap; model.vertices[11].position[2] = .65 + .28 * flap
...
animate_pelican :: proc(model: ^Aircraft_Wireframe, flap_fraction, propeller_turns: f32) {
	...
	flap := clamp_unit(flap_fraction)
	model.vertices[10].position[1] =
		1.05 - .54 * flap; model.vertices[11].position[1] = 1.05 - .54 * flap
	model.vertices[10].position[2] =
		.78 + .3 * flap; model.vertices[11].position[2] = .78 + .3 * flap
	angle := propeller_turns * 6.2831853; c := math.cos(angle) * .82; s := math.sin(angle) * .82
	model.vertices[12].position = {
		-3.275 - c,
		.72 - s,
		-.3,
	}; model.vertices[13].position = {-3.275 + c, .72 + s, -.3}
	model.vertices[14].position = {
		3.275 - c,
		.72 + s,
		-.3,
	}; model.vertices[15].position = {3.275 + c, .72 - s, -.3}
}
