...
aircraft_fleet_add :: proc(fleet: ^Aircraft_Fleet, kind: Aircraft_Kind, name: string, vehicle: ^Vehicle, available: bool) -> bool {
	if fleet == nil || vehicle == nil || fleet.count >= len(fleet.slots) do return false
	fleet.slots[fleet.count] = {kind = kind, name = name, vehicle = vehicle, available = available}
	fleet.count += 1
	return true
}
...
aircraft_kind_name :: proc(kind: Aircraft_Kind) -> string {
	...
	case .Libellula: return "LIBELLULA"
	}
	return "AIRCRAFT"
}
