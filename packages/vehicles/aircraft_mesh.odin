...
add_ring_mesh :: proc(
	...
	for side in 0 ..< sides {
		...
		mesh_triangle(
			mesh,
			front,
			ring_point(rings[0], next, sides),
			ring_point(rings[0], side, sides),
			part,
		)
		mesh_triangle(
			mesh,
...
add_section_mesh :: proc(
	...
	for index in 0 ..< len(sections) - 1 {
		...
		p0 := [3]f32 {
			...
			y - a.half_thickness,
			a.leading,
		}; p1 := [3]f32{a.x, y - a.half_thickness, a.trailing}
		p2 := [3]f32 {
			a.x,
			y + a.half_thickness,
			a.trailing,
		}; p3 := [3]f32{a.x, y + a.half_thickness, a.leading}
		q0 := [3]f32 {
			b.x,
			y - b.half_thickness,
			b.leading,
		}; q1 := [3]f32{b.x, y - b.half_thickness, b.trailing}
		q2 := [3]f32 {
			b.x,
			y + b.half_thickness,
			b.trailing,
		}; q3 := [3]f32{b.x, y + b.half_thickness, b.leading}
...
rotate_new_vertices_z :: proc(mesh: ^Aircraft_Mesh, first: int, pivot: [3]f32, angle: f32) {
	...
	for index in first ..< mesh.vertex_count {
		x :=
			mesh.vertices[index].position[0] -
			pivot[0]; y := mesh.vertices[index].position[1] - pivot[1]
...
add_propeller :: proc(
	...
	for blade in 0 ..< 3 {
		...
		add_box(
			mesh,
			{pivot[0], pivot[1] + radius * .52, pivot[2]},
			{chord, radius, thickness},
			part,
		)
...
add_horizontal_beam :: proc(
	...
	sx :=
		...
		length *
		width *
		.5; sz := dx / length * width * .5; low := -width * .5; high := width * .5
	p := [8][3]f32 {
...
pelican_hull_point :: proc(ring: Mesh_Ring, side, sides: int) -> [3]f32 {
	angle :=
		...
		2 *
		math.PI /
		f32(sides); lateral := math.cos(angle); vertical := math.sin(angle)
...
pelican_mesh :: proc() -> Aircraft_Mesh {
	...
	for x in engine_xs {
		...
		propeller_part :=
			Aircraft_Mesh_Part.Left_Propeller; if x > 0 do propeller_part = .Right_Propeller
...
libellula_mesh :: proc() -> Aircraft_Mesh {
	...
	for rotor, rotor_index in rotors {
		nacelle := [2]Mesh_Ring {
			{rotor[2] - .29, .5, rotor[1], .5},
			{rotor[2] + .29, .42, rotor[1], .42},
		}
