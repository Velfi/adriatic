...
sdf_obstacle_selected_property_edits_are_bounded_and_stable :: proc(t: ^testing.T) {
    ...
    testing.expect(
        t,
        sdf_obstacle_set_position(editor, {SDF_OBSTACLE_POSITION_LIMIT * 2, -SDF_OBSTACLE_POSITION_LIMIT * 2, 17}),
    )
...
sdf_obstacle_translation_math_and_modal_lifecycle :: proc(t: ^testing.T) {
    ...
    projection_camera := Perspective_Camera {
        position     = {},
        right        = {1, 0, 0},
        up           = {0, 1, 0},
        forward      = {0, 0, 1},
    ...
    testing.expect(
        t,
        math.abs(
            sdf_obstacle_projected_outer_radius_pixels(projection_camera, near_obstacle, 720) -
            SDF_OBSTACLE_INITIAL_RADIUS_PIXELS,
        ) <
        .001,
    )
    testing.expect(
        t,
        math.abs(
            sdf_obstacle_projected_outer_radius_pixels(projection_camera, far_obstacle, 720) -
            SDF_OBSTACLE_INITIAL_RADIUS_PIXELS,
        ) <
        .001,
    )
    ...
    testing.expect(
        t,
        math.abs((near_endpoint.position.x - near_center.position.x) - SDF_OBSTACLE_GIZMO_AXIS_PIXELS) < .001,
    )
    testing.expect(
        t,
        math.abs((far_endpoint.position.x - far_center.position.x) - SDF_OBSTACLE_GIZMO_AXIS_PIXELS) < .001,
    )
    testing.expect(
        t,
        near_ring_radius > near_size &&
        math.abs(near_ring_radius / near_size - SDF_OBSTACLE_ROTATION_RING_PIXELS / SDF_OBSTACLE_GIZMO_AXIS_PIXELS) <
            .001,
    )
    ...
    testing.expect(
        t,
        math.abs((showcase_endpoint.position.x - far_center.position.x) - SDF_OBSTACLE_GIZMO_AXIS_PIXELS) < .001,
    )
    ...
    arrow_axis, arrow_hit := sdf_obstacle_gizmo_hit_test(
        projection_camera,
        arrow_endpoint.position,
        near,
        near_size,
        1280,
        720,
    ...
    arrow_axis_at_ring, arrow_hit_at_ring := sdf_obstacle_gizmo_hit_test(
        projection_camera,
        ring_point.position,
        near,
        near_size,
    ...
    ring_axis, ring_hit := sdf_obstacle_rotation_ring_hit(
        projection_camera,
        ring_point.position,
        ring_obstacle,
        near_ring_radius,
    ...
    scale_axis, scale_hit := sdf_obstacle_scale_gizmo_hit(
        projection_camera,
    ...
    oblique_camera := Perspective_Camera {
        position     = {0, 5, 0},
        right        = {1, 0, 0},
        up           = {0, .9701425, .2425356},
        forward      = {0, -.2425356, .9701425},
        focal_length = 1.35,
    }
    ...
    oblique_ring_axis, oblique_ring_hit := sdf_obstacle_rotation_ring_hit(
        oblique_camera,
        oblique_ring_point.position,
        ring_obstacle,
        oblique_ring_radius,
        1280,
        720,
    ...
    oblique_arrow_axis, oblique_arrow_hit := sdf_obstacle_gizmo_hit_test(
        oblique_camera,
        oblique_arrow.position,
        near,
        oblique_size,
        1280,
        720,
    ...
    testing.expect(t, sdf_obstacle_modal_begin(editor, .Translate, .X))
    parallel_camera := Perspective_Camera {
        position = {3, 4, -10},
    }
    ...
    testing.expect(t, sdf_obstacle_modal_update(editor, camera, {0, 0, 1}))
    testing.expect_value(t, editor.sdf_obstacles[0].position, before.position)
    moved_camera := Perspective_Camera {
        position = {6, 8, -10},
    }
    ...
    rotate_camera := Perspective_Camera {
        position     = {3, 4, -10},
        right        = {1, 0, 0},
        up           = {0, 1, 0},
        forward      = {0, 0, 1},
    ...
    cached_rotation := linalg.quaternion_from_euler_angles(
        cached_euler.x * f32(math.PI) / 180,
        cached_euler.y * f32(math.PI) / 180,
        cached_euler.z * f32(math.PI) / 180,
        .XYZ,
    )
    testing.expect(
        t,
        math.abs(linalg.dot(editor.sdf_obstacles[0].rotation, flight.normalize_orientation(cached_rotation))) > .9999,
    )
    ...
    testing.expect_value(
        t,
        sdf_obstacle_gizmo_axis_render_color(.Translate, .Y, .X),
        sdf_obstacle_gizmo_axis_color(.X),
    )
    ...
    scale_camera := Perspective_Camera {
        position     = {3, 8, -10},
        forward      = {0, 0, 1},
        focal_length = 1.35,
    }
    ...
    testing.expect(
        t,
        math.abs(editor.sdf_obstacles[0].scale.y - axis_switch_scale.y * expected_scale_multiplier) < .001,
    )
    ...
    expected_free_multiplier := clamp(
        1 + f32(160) / SDF_OBSTACLE_FREE_SCALE_PIXELS * free_zoom_factor,
        f32(.001),
        f32(128),
    )
