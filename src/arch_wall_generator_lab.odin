...
arch_wall_generator_lab_configure :: proc(editor: ^Editor, target: string) -> bool {
    ...
    switch target {
    case "", "curve":
        arch_wall_lab_set_path(0)
    case "long":
        arch_wall_lab_set_path(1)
    case "closed", "courtyard":
        arch_wall_lab_set_path(2)
    case:
        return false
    }
    _ = lab_terrain_load(
...
world_arch_wall_generator_lab :: proc(editor: ^Editor) {
    ...
    for &span in plan.spans {
        ...
        world_box_between(
            ...
            {0, 1, 0},
            arch_wall_lab_config.thickness + .12,
            .18,
            coping,
        )
    }
    for arch in plan.arches {
        ...
        for segment in 0 ..< segments {
            ...
            world_box_rotated(
                ...
                {block_width, arch_wall_lab_config.arch_ring_thickness, arch_wall_lab_config.thickness + .14},
                yaw,
                ring,
            )
        }
        ...
        if spandrel_height > .05 {
            world_box_rotated(
                ...
                {arch.width, spandrel_height, arch_wall_lab_config.thickness},
                yaw,
                stone,
            )
        }
        ...
        for side in sides {
            ...
            world_box_rotated(
                {position.x, arch.ground + jamb_height * .5, position.y},
                {arch_wall_lab_config.arch_ring_thickness, jamb_height, arch_wall_lab_config.thickness + .14},
                yaw,
                ring,
            )
        }
...
arch_wall_generator_lab_draw_ui :: proc(editor: ^Editor, _: i32, _: i32) {
    ...
    status := fmt.ctprintf(
        "%.1f M   %d SPANS   %d ARCHES   %.1f M HIGH   %.2f M THICK",
        plan.length,
        len(plan.spans),
        len(plan.arches),
        arch_wall_lab_config.height,
