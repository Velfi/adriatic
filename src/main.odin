...
vec_sub :: proc(a, b: third_person.Vec3) -> third_person.Vec3 {return{
		a.x - b.x,
		a.y - b.y,
		a.z - b.z,
	}}
...
postale_spawn_position :: proc(editor: ^Editor) -> flight.Vec3 {
	...
	x :=
		half_extent * terrain.DEFAULT_ISLAND_OFFSET +
		half_extent * terrain.DEFAULT_RUNWAY_SPAWN_OFFSET
...
draw_instrument_dial :: proc(
	...
	needle_end := rl.Vector2 {
		center.x + math.cos(needle_angle) * 31,
		center.y + math.sin(needle_angle) * 31,
	}
	...
	rl.DrawTextEx(
		rl.Font{},
		value_text,
		{center.x - value_size.x * .5, center.y + 19},
		13,
		1,
		accent,
	)
...
project_3d :: proc(
	...
	return {
		position = {f32(width) * (.5 + x * .5), f32(height) * (.5 - y * .5)},
		depth = depth,
		visible = true,
	}
}
...
terrain_color :: proc(height, painted, sea_level: f32) -> rl.Color {
	water := rl.Color{r = 26, g = 80, b = 104, a = 255}
	sand := rl.Color{r = 205, g = 183, b = 126, a = 255}
	soil := rl.Color{r = 145, g = 101, b = 61, a = 255}
	grass := rl.Color{r = 70, g = 133, b = 80, a = 255}
...
world_under_cursor_3d :: proc(
	...
	return camera.position.x + direction.x * distance,
...
terrain_under_cursor_3d :: proc(
	...
	for distance := step; distance <= terrain.WORLD_SIZE_METERS * 2; distance += step {
		...
		if delta <= 0 && previous_delta > 0 {
			...
			return camera.position.x + direction.x * hit_distance,
				camera.position.z + direction.z * hit_distance,
				true
		}
		previous_distance = distance
...
draw_infrastructure_3d :: proc(editor: ^Editor, camera: Perspective_Camera, width, height: i32) {
	...
	for sign in terrain.DEFAULT_ISLAND_SIGNS {
		...
		draw_quad_3d(
			camera,
			{
				x = island_x - runway_half_length,
				y = runway_height,
				z = island_z - runway_half_width,
			},
			{
				x = island_x + runway_half_length,
				y = runway_height,
		...
		pier_inner_height :=
			terrain.sample_height(&editor.project, level, pier_inner_x, pier_inner_z) + .09
...
draw_terrain_3d :: proc(editor: ^Editor, width, height: i32) {
	...
	if editor.in_map {
		...
		if driving {
			...
			altitude := max(
				f32(0),
				editor.postale.body.position.y - ground - postale_game.GROUND_CLEARANCE,
			)
			hud := fmt.tprintf(
			...
			if vec_dot(delta, delta) <=
			   editor.postale.vehicle.interaction_radius *
				   editor.postale.vehicle.interaction_radius {
				rl.DrawRectangle(
					width / 2 - 116,
					height - 92,
		...
		if hit {
			...
			brush_center := project_3d(
				camera,
				{x = world_x, y = brush_height, z = world_z},
				width,
				height,
			)
...
draw_default_infrastructure :: proc(editor: ^Editor, center: rl.Vector2, scale: f32) {
	...
	for sign in terrain.DEFAULT_ISLAND_SIGNS {
		...
		a := project_point(
			island_x - runway_half_length,
			island_z - runway_half_width,
			height,
			center,
			scale,
		)
		...
		rl.DrawLineEx(
			project_point(
				island_x - runway_half_length * .82,
				island_z,
				height + .02,
				center,
				scale,
		...
		p1 := project_point(
			pier_outer_x,
			pier_outer_z - pier_width,
			editor.project.sea_level + .08,
			center,
			scale,
		)
		p2 := project_point(
...
draw_terrain_isometric_legacy :: proc(editor: ^Editor, width, height: i32, time: f32) {
	...
	draw_runway_spawn :: proc(editor: ^Editor, center: rl.Vector2, scale, x, z: f32) {
		...
		rl.DrawTextEx(
			rl.Font{},
			"SPAWN",
			{marker.x - 72, marker.y + 14},
			12,
			1,
			{r = 255, g = 234, b = 164, a = 255},
		)
	}
	...
	draw_postale_on_runway :: proc(editor: ^Editor, center: rl.Vector2, scale, x, z: f32) {
		...
		label := project_point(
			x,
			z,
			terrain.sample_height(&editor.project, 0, x, z) + 2.15,
			center,
			scale,
		)
		rl.DrawTextEx(
			rl.Font{},
			"POSTALE",
			{label.x - 28, label.y - 30},
			12,
			1,
			{r = 255, g = 239, b = 192, a = 255},
		)
	}
	for level in 0 ..< terrain.CLIPMAP_LEVELS {
		half_extent :=
			f32(terrain.RING_RESOLUTION - 1) * editor.project.levels[level].cell_size * .5
	...
	rl.DrawTextEx(
		rl.Font{},
		"ADRIATIC TERRAIN LAB",
		{20, 18},
		24,
		1,
		{r = 224, g = 245, b = 250, a = 255},
	)
	...
	rl.DrawTextEx(
		rl.Font{},
		fmt.ctprintf("%s", status),
		{20, 48},
		16,
		1,
		{r = 196, g = 222, b = 227, a = 255},
	)
	if editor.in_map {
...
draw_terrain :: proc(editor: ^Editor, width, height: i32, time: f32) {
	...
	if editor.in_map {
		...
		tool_name: cstring =
			editor.tool == .Raise ? "SCULPT" : editor.tool == .Smooth ? "SMOOTH" : "PAINT"
	...
	for !rl.WindowShouldClose() {
		...
		if editor.in_map {
			...
			editor.camera.distance = clamp(
				editor.camera.distance - rl.GetMouseWheelMove() * .5,
				3,
				12,
			)
			if driving {
				...
				if rl.GamepadAvailable() {
					control.pitch = stronger_axis(
						control.pitch,
						shape_flight_axis(rl.GetGamepadAxis(.Left_Y)),
					)
					control.roll = stronger_axis(
						control.roll,
						shape_flight_axis(rl.GetGamepadAxis(.Left_X)),
					)
					if rl.IsGamepadButtonDown(.Right_Shoulder) do control.yaw += 1
					if rl.IsGamepadButtonDown(.Left_Shoulder) do control.yaw -= 1
					control.throttle_up =
						control.throttle_up || rl.GetGamepadAxis(.Right_Trigger) > .1
					control.throttle_down =
						control.throttle_down || rl.GetGamepadAxis(.Left_Trigger) > .1
				}
