...
mouse_gait_lab_physics_step :: proc(editor: ^Editor, delta_seconds: f32) {
    if editor == nil || !mouse_gait_lab_surface_course ||
       mouse_gait_lab_moving_platform == physics.INVALID_BODY || editor.gameplay_physics.world == nil {
        return
    }
    y := f32(.36 + math.sin(editor.map_time * 1.7) * .18)
    physics.move_kinematic(
        editor.gameplay_physics.world, mouse_gait_lab_moving_platform,
        {3.55, y, 0}, physics.IDENTITY_ROTATION, max(delta_seconds, f32(1.0 / 120.0)),
    )
