...
fountain_lab_process_input :: proc(_: ^Editor) {
    ...
    if canvas2d.IsKeyPressed(.LEFT) do fountain_lab_radius = max(f32(2.2), fountain_lab_radius - .3)
    if canvas2d.IsKeyPressed(.RIGHT) do fountain_lab_radius = min(f32(7), fountain_lab_radius + .3)
    if canvas2d.IsKeyPressed(.DOWN) do fountain_lab_jets = max(0, fountain_lab_jets - 1)
    if canvas2d.IsKeyPressed(.UP) do fountain_lab_jets = min(fountains.MAX_JETS, fountain_lab_jets + 1)
    if canvas2d.IsKeyPressed(.ONE) do fountain_lab_height = max(f32(.4), fountain_lab_height - .25)
    if canvas2d.IsKeyPressed(.TWO) do fountain_lab_height = min(f32(6), fountain_lab_height + .25)
}
