...
witch_default_config :: proc() -> Witch_Broom_Config {
    return {
        ...
        attitude_gain = 8.5,
        angular_damping = 5.8,
        comfortable_acceleration = 10.5,
        max_acceleration = 24,
        linear_drag = .075,
        lateral_drag = .34,
        grip_strength = 1,
        brace_multiplier = 1.75,
        shaft_stiffness = 18,
        shaft_damping = 7,
        power_capacity = 100,
        power_recovery = 8.5,
        ground_assist = 12,
        cruise_throttle = .42,
        lateral_control_speed = 9.5,
        vertical_control_speed = 10.5,
        axis_control_gain = 2.15,
        axis_control_acceleration = 7.5,
        stick_deadzone = .11,
        camera_stick_deadzone = .19,
        trigger_deadzone = .08,
    }
}
...
witch_obstacle_avoidance_acceleration :: proc(
    position, velocity: third_person.Vec3,
    steer_intent: f32,
) -> third_person.Vec3 {
    horizontal := third_person.Vec3{velocity.x, 0, velocity.z}
...
witch_landing_assistance :: proc(
    position, velocity: third_person.Vec3,
    intent: Witch_Intent,
    mass: f32,
) -> Witch_Landing_Assist {
...
witch_update_activity :: proc(dt: f32) {
    ...
    case .Wind_Crossing:
        ...
        witch_lab.activity_complete =
            b.position.z > 72 && b.position.y > witch_ground_height(b.position.x, b.position.z) + 2
...
witch_map_left_stick :: proc(
    horizontal, vertical: f32,
    invert_horizontal, invert_vertical: bool,
) -> (
    steer, climb: f32,
) {
...
witch_lab_configure :: proc(editor: ^Editor, target: string) -> bool {
    ...
    case "",
         ...
         "ring-pass",
         "debug":
        witch_lab.activity = .Flow_Course
    case "wind", "gust":
        witch_lab.activity = .Wind_Crossing
    case "thermal":
        witch_lab.activity = .Thermal_Landing
    case "landing":
        witch_lab.activity = .Thermal_Landing
    case "recovery":
        witch_lab.activity = .Flow_Course
...
witch_sample_input :: proc() -> Witch_Input_Frame {
    ...
    if canvas2d.GamepadAvailable() {
        ...
        if math.abs(lx) + math.abs(ly) + math.abs(rx) + math.abs(ry) + rt + lt > .02 ||
           canvas2d.IsGamepadButtonDown(.Left_Shoulder) {
            steer, climb := witch_map_left_stick(lx, ly, witch_lab.invert_horizontal, witch_lab.invert_vertical)
            intent.steer = clamp(
                steer * 1.22,
                -1,
                1,
            ); intent.climb = clamp(climb * 1.18, -1, 1); intent.throttle = rt; intent.brake = lt
    ...
    if canvas2d.IsMouseButtonPressed(.LEFT) {
        ...
        frame.activity_1 =
            frame.activity_1 || canvas2d.CheckCollisionPointRec(pointer, witch_activity_button_bounds(0))
        frame.activity_2 =
            frame.activity_2 || canvas2d.CheckCollisionPointRec(pointer, witch_activity_button_bounds(1))
        frame.activity_3 =
            frame.activity_3 || canvas2d.CheckCollisionPointRec(pointer, witch_activity_button_bounds(2))
    ...
    if keyboard_active ||
       ...
       keyboard_replay ||
       keyboard_debug ||
       frame.toggle_horizontal ||
       frame.toggle_vertical {
...
witch_step :: proc(dt, time: f32) {
    ...
    witch_lab.flight_intent.throttle = witch_approach(
        witch_lab.flight_intent.throttle,
        witch_lab.intent.throttle,
        dt * 2.6,
    )
    ...
    b.angular_velocity.x +=
        ((desired_pitch - b.pitch) * c.attitude_gain - b.angular_velocity.x * c.angular_damping) * dt
    ...
    b.angular_velocity.y +=
        (intent.steer * c.yaw_torque / yaw_inertia * yaw_authority - b.angular_velocity.y * 2.35) * dt
    ...
    up := third_person.Vec3 {
        -right.x * f32(math.sin(f64(b.roll))),
        f32(math.cos(f64(b.roll))),
        -right.z * f32(math.sin(f64(b.roll))),
    }
    ...
    b.desired_acceleration =
        up * (lift / mass - 9.81) + forward * (thrust / mass) + right * lateral_control_acceleration
    ...
    if danger >
       1.15 { b.grip -= (danger - 1.15) * .13 * dt } else { b.grip += (intent.brace ? f32(.17) : f32(.08)) * dt }
    ...
    on_landing :=
        math.abs(b.position.x - 42) < 18 && math.abs(b.position.z - 132) < 13 && b.position.y < landing_ground + 1.5
...
witch_update_camera :: proc(editor: ^Editor) {
    ...
    if witch_lab.camera_obstruction_snap {
        ...
        response :=
            witch_length(obstruction_target) > witch_length(witch_lab.camera_obstruction_offset) ? f32(8.5) : f32(5.5)
...
witch_camera_avoid_trees :: proc(follow, desired_eye: third_person.Vec3) -> third_person.Vec3 {
    ...
    for tree in witch_trees {
        ...
        projection := clamp(
            (to_center.x * ray.x + to_center.y * ray.y + to_center.z * ray.z) / (ray_length * ray_length),
            0,
            1,
        )
    ...
    for tree in witch_trees {
        ...
        to_center := center - follow
        projection := clamp(
            (to_center.x * ray.x + to_center.y * ray.y + to_center.z * ray.z) / ray_length_squared,
            0,
            1,
        )
        closest := follow + ray * projection
...
witch_draw_ring :: proc(center, direction: third_person.Vec3, radius: f32, color: canvas2d.Color) {
    ...
    for segment in 0 ..< 24 {
        ...
        p0 :=
            center + right * (f32(math.cos(f64(a))) * radius) + third_person.Vec3{0, f32(math.sin(f64(a))) * radius, 0}
        p1 :=
            center +
            right * (f32(math.cos(f64(next_angle))) * radius) +
            third_person.Vec3{0, f32(math.sin(f64(next_angle))) * radius, 0}
...
witch_draw_ring_pass :: proc() {
    ...
    for spark in 0 ..< 12 {
        ...
        color :=
            spark & 1 == 0 ? canvas2d.Color{120, 236, 192, u8(220 * fade)} : canvas2d.Color{255, 224, 112, u8(210 * fade)}
...
witch_draw_witch :: proc() {
    ...
    for bristle in -3 ..= 3 {
        ...
        tip := witch_transform(
            {offset * (1 + flare) + bristle_chatter, -.10 - math.abs(offset) * .15, -3.7 - b.shaft_flex * .25},
        )
        world_tube_between(root, tip, {0, 1, 0}, .065, .035, {145, 103, 48, 255})
    }
    brace :=
        pose_broom.rider_state == .Bracing ||
        pose_broom.rider_state == .Straining ||
        pose_broom.rider_state == .Critical
    ...
    head := witch_transform(
        {lean + slip * 1.3 + recovery_side + gust_look, 1.92 - crouch + recovery_lift, pose.head_fore_aft},
    )
    world_tube_between(hips, torso, forward, .32, .26, {88, 48, 126, 255})
    world_tube_between(torso, head, forward, .24, .22, {231, 178, 146, 255})
    hat_tip := witch_transform(
        {lean + slip * 1.3 + recovery_side + gust_look, 2.62 - crouch + recovery_lift, -.20 - b.wind.x * .012},
    )
    ...
    for side in sides {
        ...
        knee := witch_transform(
            {side * .32 + recovery_side, .45 - crouch * .7 + recovery_lift - landing_pose * .10, -.18},
        )
        foot := witch_transform(
            {side * .24 + recovery_side, .14 + recovery_lift - landing_pose * .30, brace ? .65 : .22},
        )
    ...
    scarf_tip :=
        scarf_root -
        forward * (1.1 + witch_length(b.velocity - b.wind) * .035) +
        third_person.Vec3{-b.wind.x * .025, .05, 0}
    ...
    world_tube_between(
        witch_transform({0, -.02, -2.55}),
        witch_transform({0, -.02, -2.9}),
        {0, 1, 0},
        .18 + glow_flicker,
        .08,
        glow,
    ...
    if witch_lab.activity == .Wind_Crossing {
        ...
        world_box(
            parcel,
            {.66, .5, .72},
            witch_lab.parcel_security > .45 ? canvas2d.Color{205, 151, 73, 255} : canvas2d.Color{226, 105, 75, 255},
        )
...
witch_draw_flight_feedback :: proc() {
    ...
    world_tube_between(
        center + third_person.Vec3{0, .46, 0},
        center + third_person.Vec3{0, 1.05, 0},
        forward,
        .055,
        .055,
        color,
...
witch_draw_completion_burst :: proc() {
    ...
    for spark in 0 ..< 14 {
        ...
        direction := witch_safe_normalize(
            third_person.Vec3{f32(math.cos(f64(angle))), lift, f32(math.sin(f64(angle)))},
            {1, 0, 0},
        )
        start := center + direction * expansion
        color :=
            spark & 1 == 0 ? canvas2d.Color{255, 222, 103, u8(220 * fade)} : canvas2d.Color{120, 231, 204, u8(220 * fade)}
...
witch_draw_physics_debug :: proc() {
    ...
    if torque_length > .001 {
        ...
        world_tube_between(
            p,
            p + torque_direction * min(torque_length * .018, f32(3)),
            {0, 1, 0},
            .065,
            .025,
            {209, 120, 255, 255},
...
witch_draw_beacon :: proc(center: third_person.Vec3, color: canvas2d.Color) {
    ...
    world_tube_between(
        center + third_person.Vec3{0, .1, 0},
        center + third_person.Vec3{0, 8, 0},
        {1, 0, 0},
        .09,
        .09,
        color,
...
world_witch_lab :: proc(editor: ^Editor) {
    ...
    if witch_lab.activity == .Flow_Course {
        for center, index in witch_ring_positions {
            ...
            color :=
                index < witch_lab.ring_index ? canvas2d.Color{112, 225, 155, 255} : (index == witch_lab.ring_index ? canvas2d.Color{255, 220, 92, 255} : canvas2d.Color{169, 133, 220, 255})
...
witch_rider_state_name :: proc(state: Witch_Rider_State) -> cstring {
    switch state {case .Comfortable:
        return "COMFORTABLE"; case .Bracing:
        return "BRACING"; case .Straining:
        return "STRAINING"; case .Slipping:
        return "SLIPPING"; case .Critical:
        return "HANGING ON"; case .Recovering:
        return "RECOVERING"}
    return ""
}

witch_activity_name :: proc(activity: Witch_Activity) -> cstring {
    switch activity {case .Flow_Course:
        return "FLY THE MEADOW CIRCUIT"; case .Wind_Crossing:
        return "CARRY THE PARCEL ACROSS THE WIND"; case .Thermal_Landing:
        return "RIDE THE THERMAL, THEN LAND"}
    return ""
}
...
witch_activity_status :: proc() -> cstring {
    ...
    if witch_lab.activity_complete {
        ...
        case .Wind_Crossing:
            return witch_lab.parcel_security > .55 ? "PARCEL DELIVERED" : "PARCEL DELIVERED — ROUGH RIDE"
        case .Thermal_Landing:
            return "THERMAL LANDING COMPLETE"
        }
    }
...
witch_draw_axis_toggle :: proc(bounds: canvas2d.Rectangle, label: cstring, inverted: bool) {
    ...
    canvas2d.DrawRectangleRoundedLinesEx(
        ...
        6,
        1,
        inverted ? canvas2d.Color{123, 224, 207, 255} : canvas2d.Color{111, 126, 142, 255},
    )
    state: cstring = inverted ? "REVERSED" : "NORMAL"
    canvas2d.DrawTextEx(
        canvas2d.Font{},
        fmt.ctprintf("%s  %s", label, state),
...
witch_draw_activity_button :: proc(index: int, label: cstring) {
    ...
    canvas2d.DrawRectangleRoundedLinesEx(
        ...
        6,
        1,
        selected ? canvas2d.Color{177, 225, 187, 255} : canvas2d.Color{102, 115, 128, 240},
    )
    size := canvas2d.MeasureTextEx(canvas2d.Font{}, label, 11, 1)
    canvas2d.DrawTextEx(
        canvas2d.Font{},
        label,
        {bounds.x + (bounds.width - size.x) * .5, bounds.y + 9},
        11,
        1,
        {236, 233, 209, 255},
    )
...
witch_lab_draw_ui :: proc(_: ^Editor, width, height: i32) {
    ...
    canvas2d.DrawTextEx(
        canvas2d.Font{},
        witch_activity_name(witch_lab.activity),
        {38, 69},
        13,
        1,
        {215, 222, 232, 255},
    ...
    canvas2d.DrawTextEx(
        ...
        12,
        1,
        b.grip < .3 ? canvas2d.Color{255, 142, 110, 255} : canvas2d.Color{139, 231, 180, 255},
    )
    ...
    canvas2d.DrawTextEx(
        canvas2d.Font{},
        prompt,
        {f32(width) * .5 - size.x * .5, f32(height) - 34},
        12,
        1,
        {245, 237, 207, 255},
    )
    ...
    if witch_lab.activity_complete {
        ...
        canvas2d.DrawRectangleRounded(
            ...
            .18,
            7,
            hovered ? canvas2d.Color{91, 112, 98, 245} : canvas2d.Color{64, 84, 75, 238},
        )
        ...
        canvas2d.DrawTextEx(
            canvas2d.Font{},
            label,
            {replay_bounds.x + (replay_bounds.width - label_size.x) * .5, replay_bounds.y + 10},
            12,
            1,
            {244, 239, 205, 255},
        )
    ...
    if witch_lab.debug {
        ...
        debug_lines := [5]cstring {
            fmt.ctprintf(
                "SPEED %.1f  AIR %.1f  ALT %.1f  WIND %.1f %.1f %.1f",
                witch_length(b.velocity),
                witch_length(b.velocity - b.wind),
                b.position.y - ground,
                b.wind.x,
                b.wind.y,
                b.wind.z,
            ...
            fmt.ctprintf(
                "INTENT S %.2f  C %.2f  T %.2f  B %.2f",
                witch_lab.intent.steer,
                witch_lab.intent.climb,
                witch_lab.intent.throttle,
                witch_lab.intent.brake,
            ),
            fmt.ctprintf(
                "DES ACC %.1f %.1f %.1f  ACT %.1f %.1f %.1f",
                b.desired_acceleration.x,
                b.desired_acceleration.y,
                b.desired_acceleration.z,
                b.actual_acceleration.x,
                b.actual_acceleration.y,
                b.actual_acceleration.z,
            ),
            fmt.ctprintf(
                "POWER %.1f  GRIP %.2f  %s  BRACE %s",
                b.power,
                b.grip,
                witch_rider_state_name(b.rider_state),
                brace,
            ),
            fmt.ctprintf(
                "TORQUE %.1f %.1f %.1f  DT %.4f",
                b.net_torque.x,
                b.net_torque.y,
                b.net_torque.z,
                WITCH_FIXED_DT,
...
witch_tree_impact_is_recoverable :: proc(t: ^testing.T) {
    ...
    testing.expect(
        t,
        witch_length(third_person.Vec3{witch_lab.broom.position.x - tree.x, 0, witch_lab.broom.position.z - tree.z}) >=
        .65 * tree.scale + .54,
    )
...
witch_activities_complete_from_simulation_state :: proc(t: ^testing.T) {
    witch_lab = {
        config     = witch_default_config(),
        activity   = .Flow_Course,
        ring_index = WITCH_RING_COUNT - 1,
    }
    ...
    witch_lab = {
        config          = witch_default_config(),
        activity        = .Thermal_Landing,
        thermal_reached = true,
        landed          = true,
    }
    witch_update_activity(WITCH_FIXED_DT)
    testing.expect(t, witch_lab.activity_complete)
...
witch_full_stick_has_turn_climb_and_dive_authority :: proc(t: ^testing.T) {
    ...
    testing.expectf(
        t,
        math.abs(witch_lab.broom.yaw - start_yaw) > .45,
        "full steer changed heading by only %.3f rad",
        math.abs(witch_lab.broom.yaw - start_yaw),
    )
    ...
    testing.expectf(
        t,
        witch_lab.broom.velocity.y > 2.5,
        "full climb produced only %.2f m/s vertical speed",
        witch_lab.broom.velocity.y,
    )
    ...
    testing.expectf(
        t,
        witch_lab.broom.velocity.y < -2.5,
        "full dive produced only %.2f m/s vertical speed",
        witch_lab.broom.velocity.y,
    )
}
...
witch_assisted_recovery_restores_bounded_grip :: proc(t: ^testing.T) {
    ...
    witch_lab.broom = {
        ...
        grip          = .05,
        recovery_time = 2.2,
        rider_state   = .Recovering,
    }
    for step in 0 ..< 360 do witch_step(WITCH_FIXED_DT, f32(step) * WITCH_FIXED_DT)
    testing.expect(t, witch_lab.broom.recovery_time <= 0)
    testing.expectf(
        t,
        witch_lab.broom.grip > .5 && witch_lab.broom.grip <= 1,
        "recovery left grip outside healthy bounds: %.3f",
        witch_lab.broom.grip,
    )
...
witch_terrain_assist_reads_ridge_ahead_and_respects_descent :: proc(t: ^testing.T) {
    ...
    testing.expectf(
        t,
        descending < assisted * .4,
        "deliberate descent retained too much assistance: %.2f vs %.2f",
        descending,
        assisted,
    )
}
...
witch_obstacle_assist_deflects_but_yields_to_player_steer :: proc(t: ^testing.T) {
    ...
    testing.expectf(
        t,
        witch_length(assisted) > .25,
        "approaching tree produced only %.3f m/s2 avoidance",
        witch_length(assisted),
    )
    testing.expectf(
        t,
        witch_length(overridden) < witch_length(assisted) * .5,
        "full steer did not retain authority: %.3f vs %.3f",
        witch_length(overridden),
        witch_length(assisted),
    )
}
...
witch_landing_assist_softens_descent_and_allows_go_around :: proc(t: ^testing.T) {
    ...
    testing.expectf(
        t,
        go_around.factor < assisted.factor * .2,
        "throttle did not release landing assist: %.3f vs %.3f",
        go_around.factor,
        assisted.factor,
    )
}
...
witch_course_gates_face_the_route :: proc(t: ^testing.T) {
    for index in 0 ..< WITCH_RING_COUNT {
        ...
        testing.expectf(
            t,
            math.abs(witch_length(direction) - 1) < .001,
            "gate %d direction was not normalized",
            index + 1,
        )
        testing.expectf(
            t,
            math.abs(direction.y) < .001,
            "gate %d tilted out of its readable vertical plane",
            index + 1,
        )
        if index + 1 < WITCH_RING_COUNT {
            ...
            testing.expectf(
                t,
                direction.x * route.x + direction.z * route.z >= 0,
                "gate %d faced away from the route",
                index + 1,
            )
        }
    }
...
witch_neutral_cruise_advances_and_brake_slows :: proc(t: ^testing.T) {
    ...
    testing.expectf(
        t,
        witch_lab.broom.position.z > start_z + 7,
        "neutral cruise advanced only %.2f m",
        witch_lab.broom.position.z - start_z,
    )
    ...
    testing.expectf(
        t,
        braking_speed < cruise_speed * .7,
        "braking only reduced speed from %.2f to %.2f m/s",
        cruise_speed,
        braking_speed,
    )
}
...
witch_exhaustion_keeps_control_and_causes_only_a_gentle_sink :: proc(t: ^testing.T) {
    ...
    testing.expectf(
        t,
        witch_lab.broom.velocity.y < -.2 && witch_lab.broom.velocity.y > -3,
        "exhaustion vertical speed was %.2f m/s",
        witch_lab.broom.velocity.y,
    )
    testing.expectf(
        t,
        witch_lab.broom.velocity.z > 10,
        "exhausted broom lost forward control at %.2f m/s",
        witch_lab.broom.velocity.z,
    )
...
witch_arcade_turn_banks_changes_heading_and_settles :: proc(t: ^testing.T) {
    ...
    testing.expectf(
        t,
        math.abs(witch_lab.broom.roll) > .12 && math.abs(witch_lab.broom.roll) < .75,
        "turn bank reached %.2f rad",
        witch_lab.broom.roll,
    )
    testing.expectf(
        t,
        witch_lab.broom.position.x < -.8,
        "left turn moved only %.2f m left",
        witch_lab.broom.position.x,
    )
    ...
    testing.expectf(
        t,
        math.abs(witch_lab.broom.roll) < .16,
        "released turn retained %.2f rad bank",
        witch_lab.broom.roll,
    )
    testing.expectf(
        t,
        math.abs(witch_lab.broom.angular_velocity.y) < .22,
        "released turn retained %.2f rad/s yaw",
        witch_lab.broom.angular_velocity.y,
    )
}
...
witch_chase_camera_dodges_tree_obstructions :: proc(t: ^testing.T) {
    ...
    testing.expectf(
        t,
        math.abs(corrected.x - obstructed_eye.x) > 3,
        "camera dodged only %.2f m sideways",
        math.abs(corrected.x - obstructed_eye.x),
    )
    testing.expectf(
        t,
        witch_length(corrected - follow) > witch_length(obstructed_eye - follow) * .75,
        "camera collapsed to %.2f m from rider",
        witch_length(corrected - follow),
    )
    ...
    testing.expectf(
        t,
        witch_length(clear - clear_eye) < .001,
        "clear camera path moved by %.3f m",
        witch_length(clear - clear_eye),
    )
}
...
witch_stick_axes_correct_quickly_and_recenter_velocity :: proc(t: ^testing.T) {
    ...
    testing.expectf(
        t,
        witch_lab.broom.position.x < -.55,
        "half-second horizontal correction moved only %.2f m",
        witch_lab.broom.position.x,
    )
    witch_lab.intent.steer = 0
    ...
    testing.expectf(
        t,
        math.abs(lateral_speed) < 1.2,
        "released horizontal input retained %.2f m/s side slip",
        lateral_speed,
    )
    ...
    testing.expectf(
        t,
        witch_lab.broom.velocity.y > 3,
        "half-second vertical correction reached only %.2f m/s",
        witch_lab.broom.velocity.y,
    )
    ...
    testing.expectf(
        t,
        math.abs(witch_lab.broom.velocity.y) < 1.2,
        "released vertical input retained %.2f m/s",
        witch_lab.broom.velocity.y,
    )
}
...
witch_completion_replay_is_immediate_and_preserves_options :: proc(t: ^testing.T) {
    ...
    testing.expectf(
        t,
        witch_lab.completion_time == .5,
        "completion acknowledgement advanced %.2f s",
        witch_lab.completion_time,
    )
    witch_activity_start(.Flow_Course)
...
witch_ring_pass_feedback_starts_at_crossed_gate_and_expires :: proc(t: ^testing.T) {
    ...
    testing.expectf(
        t,
        witch_lab.ring_pass_time > .6,
        "ring feedback started with only %.2f s",
        witch_lab.ring_pass_time,
    )
