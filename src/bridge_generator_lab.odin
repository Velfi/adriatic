...
bridge_lab_archetype_name :: proc(value: bridges.Archetype) -> cstring {
    switch value {
    case .Dalmatian_Multi_Arch:       return "DALMATIAN MULTI-ARCH"
    case .Herzegovinian_High_Arch:   return "HERZEGOVINIAN HIGH ARCH"
    case .Venetian_Canal:             return "VENETIAN CANAL"
    case .Cycladic_Rural:             return "CYCLADIC RURAL"
    case .Aegean_Fortress:            return "AEGEAN FORTRESS APPROACH"
    case .Timber_Trestle:             return "TIMBER TRESTLE"
    case .Iron_Truss:                 return "IRON TRUSS"
    }
    return "BRIDGE"
}

bridge_lab_region_name :: proc(value: bridges.Region) -> cstring {
    switch value {case .Adriatic: return "ADRIATIC"; case .Aegean: return "AEGEAN"; case .Later_Era: return "LATER ERA"}
    return "REGION"
}
...
bridge_lab_material_name :: proc(value: bridges.Material) -> cstring {
    ...
    case .Istrian_Stone: return "ISTRIAN STONE"
...
bridge_generator_lab_configure :: proc(editor: ^Editor, target: string) -> bool {
    ...
    case "herzegovinian", "high-arch", "mostar": bridge_lab_apply_archetype(.Herzegovinian_High_Arch)
    case "venetian", "canal": bridge_lab_apply_archetype(.Venetian_Canal)
    case "rialto":
    ...
    case "aegean", "cycladic", "rural": bridge_lab_apply_archetype(.Cycladic_Rural)
    case "fortress", "andros": bridge_lab_apply_archetype(.Aegean_Fortress)
    case "timber", "trestle": bridge_lab_apply_archetype(.Timber_Trestle)
    case "iron", "truss": bridge_lab_apply_archetype(.Iron_Truss)
    case: return false
    }
    ...
    case .Herzegovinian_High_Arch: camera_eye, camera_target = {25,14,22}, {0,5.2,0}
    case .Venetian_Canal: camera_eye, camera_target = {20,10,18}, {0,3.4,0}
    case .Cycladic_Rural: camera_eye, camera_target = {14,8,13}, {0,2.9,0}
    case .Aegean_Fortress: camera_eye, camera_target = {19,11,17}, {0,4.0,0}
    case .Timber_Trestle, .Iron_Truss: camera_eye, camera_target = {27,14,24}, {0,4.0,0}
...
bridge_generator_lab_process_input :: proc(editor: ^Editor) {
    ...
    if canvas2d.IsKeyPressed(.R) {
        ...
        case .Adriatic: bridge_lab_apply_archetype(.Cycladic_Rural)
        case .Aegean: bridge_lab_apply_archetype(.Timber_Trestle)
        case .Later_Era: bridge_lab_apply_archetype(.Dalmatian_Multi_Arch)
        }
        terrain_changed = true
    }
...
bridge_lab_scale_color :: proc(color: canvas2d.Color, scale: f32) -> canvas2d.Color {
    return {u8(clamp(f32(color.r)*scale, 0, 255)), u8(clamp(f32(color.g)*scale, 0, 255)), u8(clamp(f32(color.b)*scale, 0, 255)), color.a}
}

bridge_lab_palette :: proc(plan: ^bridges.Plan) -> (body, ring, paving, accent: canvas2d.Color) {
    switch plan.material {
    case .Limestone: return {151,143,126,255}, {188,179,157,255}, {126,121,108,255}, {78,66,50,255}
    case .Travertine: return {159,148,124,255}, {188,173,142,255}, {131,123,106,255}, {88,76,57,255}
    case .Istrian_Stone: return {205,199,178,255}, {235,229,207,255}, {173,166,148,255}, {113,86,55,255}
    case .Slate: return {101,106,102,255}, {137,142,137,255}, {83,88,86,255}, {65,60,52,255}
    case .Fieldstone: return {137,128,105,255}, {164,154,127,255}, {111,104,89,255}, {75,66,48,255}
    case .Timber: return {91,64,41,255}, {124,88,52,255}, {74,55,39,255}, {49,45,39,255}
    case .Iron: return {61,67,64,255}, {88,96,92,255}, {78,75,67,255}, {43,46,44,255}
    }
    return {}, {}, {}, {}
}
...
bridge_lab_profile_rise :: proc(plan: ^bridges.Plan, x: f32) -> f32 {
    ...
    case .Crowned: return plan.deck_rise*(1-normalized*normalized)
    case .Humpback: return plan.deck_rise*(1-normalized)
    case .Stepped:
        steps := f32(8)
...
bridge_lab_arch_fraction :: proc(plan: ^bridges.Plan, u: f32) -> f32 {
    ...
    case .Semicircular, .Segmental: return math.sqrt(max(f32(0), 1-u*u))
...
bridge_lab_spandrel_strip :: proc(
    x0, x1, opening_y0, opening_y1, top_y0, top_y1, width: f32,
    color: canvas2d.Color,
) {
    z := width*.5
    p := [8]third_person.Vec3{
        {x0,opening_y0,-z}, {x1,opening_y1,-z},
        {x1,top_y1,-z},     {x0,top_y0,-z},
        {x0,opening_y0,z},  {x1,opening_y1,z},
        {x1,top_y1,z},      {x0,top_y0,z},
    }
...
bridge_lab_masonry :: proc(plan: ^bridges.Plan, body, ring: canvas2d.Color) {
    ...
    for span in 0 ..< plan.pier_count+1 {
        ...
        for index in 0 ..< pieces {
            ...
            shade := 1 + (f32(bridges.mix(plan.seed+u32(span*pieces+index))&255)/255-.5)*plan.masonry_variation
            bridge_lab_spandrel_strip(
                x0,x1,
                min(opening_y0,underside0-.02),min(opening_y1,underside1-.02),
                underside0,underside1,plan.width,
        ...
        for side in sides {
            ...
            for index in 1 ..= pieces {
                ...
                point := third_person.Vec3{center_x+u*half_opening, spring_y+bridge_lab_arch_fraction(plan,u)*plan.arch_rise, z}
    ...
    if terminal_inner < bridge_half {
        bridge_lab_spandrel_strip(
            -terminal_outer,-terminal_inner,0,0,
        ...
        bridge_lab_spandrel_strip(
            terminal_inner,terminal_outer,0,0,
            bridge_lab_deck_surface(plan,terminal_inner)-plan.deck_thickness,
            bridge_lab_deck_surface(plan,bridge_half)-plan.deck_thickness,
            terminal_width,body,
        )
...
bridge_lab_piers :: proc(plan: ^bridges.Plan, body, accent: canvas2d.Color) {
    for pier in plan.piers[:plan.pier_count] {
        ...
        if plan.pier_cutwaters {
            world_box_rotated({pier.station,top*.32,-plan.width*.48},{pier.width*.82,top*.60,pier.width*.82},math.PI*.25,accent)
            world_box_rotated({pier.station,top*.32,plan.width*.48},{pier.width*.82,top*.60,pier.width*.82},math.PI*.25,accent)
        }
    }
}
    ...
    for side in sides {
        ...
        if plan.parapet == .Balustrade || plan.parapet == .Rail {
            for index in 0 ..= pieces {
                ...
                world_box({x,surface+(plan.parapet_height-foundation_depth)*.5,z},{plan.parapet_width,post_height,plan.parapet_width},color)
            }
            for index in 0 ..< pieces {
                x0 := -plan.length*.5+f32(index)*plan.length/f32(pieces)
                x1 := -plan.length*.5+f32(index+1)*plan.length/f32(pieces)
                bridge_lab_member({x0,bridge_lab_deck_surface(plan,x0)+plan.parapet_height,z},{x1,bridge_lab_deck_surface(plan,x1)+plan.parapet_height,z},plan.parapet_width,color)
            }
        } else {
            for index in 0 ..< pieces {
                x0 := -plan.length*.5+f32(index)*plan.length/f32(pieces)
                x1 := -plan.length*.5+f32(index+1)*plan.length/f32(pieces)
                xm := (x0+x1)*.5
                irregular := plan.parapet == .Low_Irregular ? (f32(bridges.mix(plan.seed+u32(index))&255)/255-.5)*.16 : f32(0)
                height := plan.parapet_height+irregular
                grounded_height := height+foundation_depth
                world_box({xm,bridge_lab_deck_surface(plan,xm)+(height-foundation_depth)*.5,z},{(x1-x0)*1.02,grounded_height,plan.parapet_width},color)
            }
        }
    }
}
...
bridge_lab_trusses :: proc(plan: ^bridges.Plan, color: canvas2d.Color) {
    ...
    for side in sides {
        ...
        bridge_lab_member({-plan.length*.5,deck_y+plan.truss_height,z},{plan.length*.5,deck_y+plan.truss_height,z},.16,color)
        ...
        for index in 0 ..< bays {
            ...
            bridge_lab_member({x0,index&1==0?deck_y:deck_y+plan.truss_height,z},{x1,index&1==0?deck_y+plan.truss_height:deck_y,z},.12,color)
        }
    }
}
...
bridge_lab_trestles :: proc(plan: ^bridges.Plan, color: canvas2d.Color) {
    ...
    for span in 0 ..< plan.pier_count+1 {
        ...
        for side in sides {
            ...
            bridge_lab_member({x-plan.span_length*.35,0,z},{x+plan.span_length*.35,plan.clearance,z},.24,color)
            bridge_lab_member({x+plan.span_length*.35,0,z},{x-plan.span_length*.35,plan.clearance,z},.24,color)
        }
    }
}
...
bridge_generator_lab_draw_ui :: proc(_: ^Editor, _: i32, _: i32) {
    ...
    status := fmt.ctprintf("%s   %s   %s   %.0f M   %d SPAN%s",bridge_lab_region_name(plan.region),bridge_lab_archetype_name(plan.archetype),bridge_lab_material_name(plan.material),plan.length,span_count,span_count==1?"":"S")
    canvas2d.DrawTextEx(canvas2d.Font{},status,{38,72},13,1,{190,213,189,255})
    canvas2d.DrawTextEx(canvas2d.Font{},"A / D VARIANT   S ARCHETYPE   R REGION   LEFT / RIGHT LENGTH   UP / DOWN SPANS",{38,102},12,1,{199,198,177,255})
    secondary_controls: cstring = bridge_lab_config.archetype == .Venetian_Canal ? "1 / 2 CLEARANCE   C CUTWATERS   U SHOPS" : "1 / 2 CLEARANCE   C CUTWATERS"
    canvas2d.DrawTextEx(canvas2d.Font{},secondary_controls,{38,130},12,1,{199,198,177,255})
    cleft_status := fmt.ctprintf("Q / E CLEFT WIDTH %.0f M   Z / X CLEFT DEPTH %.1f M",bridge_lab_cleft_width,bridge_lab_cleft_depth)
