...
player_mail_collect :: proc(editor: ^Editor) -> int {
    ...
    if story.has_friendometer(&editor.story_state) &&
       player_mail.receive(&editor.player_mail, .Mirna_Field_Notes) {
        collected += 1
    }
    if editor.story_state.completed_deliveries >= 3 &&
       player_mail.receive(&editor.player_mail, .Postmasters_Thanks) {
        collected += 1
    }
...
player_mail_draw :: proc(editor: ^Editor, width, height: i32) {
    ...
    ui_draw_text(.Data, fmt.ctprintf("%d UNREAD", unread), {panel.x + panel.width - 112, panel.y + 58}, .3, ui_theme_accent())

    ids: [player_mail.LETTER_COUNT]player_mail.Letter_ID
    count := player_mail_received_ids(editor, &ids)
    if count == 0 {
        ui_draw_text(.Body, "No letters collected yet. Check at either island post office.", {list.x + 12, list.y + 18}, .2, ui_theme_text_muted())
    } else {
    ...
    ui_draw_text(.Data, hint, {panel.x + panel.width - size.x - 28, panel.y + panel.height - 18}, .2, ui_theme_text_muted())
}
