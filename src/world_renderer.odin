...
world_renderer_create_pipelines :: proc(
    ctx: ^engine.Vk_Context,
    ...
    if !render3d.create_color_pipeline_variants(ctx, &info, .D32_SFLOAT, pipelines) do return false
    ...
    info.layout = sky_layout
    if !render3d.create_color_pipeline_variants(ctx, &info, .D32_SFLOAT, sky_pipelines) do return false
    return true
}
...
world_renderer_create :: proc(ctx: ^engine.Vk_Context) -> bool {
    ...
    if !world_renderer_create_pipelines(
        ctx,
        world_renderer.layout,
        world_renderer.sky_layout,
        &world_renderer.pipelines,
        &world_renderer.sky_pipelines,
    ) {
        render3d.destroy_color_pipeline_variants(ctx, &world_renderer.pipelines)
        render3d.destroy_color_pipeline_variants(ctx, &world_renderer.sky_pipelines)
        vk.DestroyPipelineLayout(ctx.device, world_renderer.layout, nil)
        vk.DestroyPipelineLayout(ctx.device, world_renderer.sky_layout, nil)
        world_renderer.layout = {}
        world_renderer.sky_layout = {}
        return false
    }
...
world_renderer_reload :: proc() -> bool {
    if !world_renderer.initialized || world_renderer.ctx == nil do return false
    _ = vk.DeviceWaitIdle(world_renderer.ctx.device)
    next, next_sky: [render3d.COLOR_PIPELINE_VARIANT_COUNT]vk.Pipeline
    if !world_renderer_create_pipelines(
        world_renderer.ctx,
        world_renderer.layout,
        world_renderer.sky_layout,
        ...
        render3d.destroy_color_pipeline_variants(world_renderer.ctx, &next)
        render3d.destroy_color_pipeline_variants(world_renderer.ctx, &next_sky)
        return false
    }
    ...
    world_renderer.pipelines = next
    world_renderer.sky_pipelines = next_sky
    return true
}
