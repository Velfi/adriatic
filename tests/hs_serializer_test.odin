...
hs_serializer_body_field_equal :: proc(data: []byte, body_start: int, offset: uintptr, value: rawptr, size: int) -> bool {
...
hs_serializer_fixed_leaf_arrays_preserve_wire_and_dynamic_siblings :: proc(t: ^testing.T) {
    ...
    testing.expect(t, hs_serializer_body_field_equal(
        encoded,
        body_start,
    ...
    testing.expect(t, hs_serializer_bytes_equal(mem.ptr_to_bytes(&destination.floats), mem.ptr_to_bytes(&source.floats)))
    testing.expect(t, destination.nested.bytes == source.nested.bytes)
    testing.expect(t, hs_serializer_bytes_equal(
