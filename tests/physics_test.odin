...
@(test)
physics_soft_strand_attachment_can_change_heading :: proc(t: ^testing.T) {
    world := physics.create_world(128, 1)
    defer physics.destroy_world(world)
    points := [5]physics.Vec3{{0, 2, 0}, {0, 2, .2}, {0, 2, .4}, {0, 2, .6}, {0, 2, .8}}
    inverse_masses := [5]f32{0, 0, 1, 1, 1}
    ...
    for _ in 0 ..< 30 {
        testing.expect(t, physics.set_soft_strand_attachment(world, strand, {0, 2, 0}, {.2, 2, 0}, 1.0 / 120.0))
        physics.step(world, 1.0 / 120.0, 2)
    }
    ...
    testing.expect(t, physics.get_soft_strand_points(world, strand, output[:]))
    testing.expect(t, math.abs(output[0].x) < .001)
    testing.expect(t, math.abs(output[0].z) < .001)
    testing.expectf(
        t,
        math.abs(output[1].x - .2) < .01 && math.abs(output[1].z) < .01,
        "turned attachment was (%.3f, %.3f, %.3f)",
        output[1].x,
        output[1].y,
        output[1].z,
    )
    ...
    testing.expectf(
        t,
        math.abs(tail_offset.x) > math.abs(tail_offset.z) * 2,
        "rod frames did not follow turned attachment: tail offset (%.3f, %.3f, %.3f)",
        tail_offset.x,
        tail_offset.y,
        tail_offset.z,
    )
