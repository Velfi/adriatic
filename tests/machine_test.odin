...
machine_catalog_ports_the_fleet_maintenance_contracts :: proc(t: ^testing.T) {
    ...
    testing.expect(t, procedure.step.montage_repetitions == 3 && procedure.verification.handle_id == "leak_test")
}
...
physical_repair_requires_a_verification_gesture :: proc(t: ^testing.T) {
    ...
    testing.expect(t, machines.advance(&active, procedure, &verification, 1, 1) == .Repair_Completed)
